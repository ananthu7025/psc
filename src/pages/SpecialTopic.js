...
const SpecialTopic = () => {
  ...
  const handleUpload = () => {
    ...
    const uploadUrl = `${BASE_URL}/Special_Topic/upload?subfolder=${encodeURIComponent(subfolder)}`;
  ...
  const handleOpenPDF = (subfolder, file) => {
    const pdfUrl = `${BASE_URL}/Special_Topic/pdfs/${encodeURIComponent(subfolder)}/${encodeURIComponent(file)}`;
